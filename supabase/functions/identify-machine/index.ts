  ...
  try{
    ...
    const response=await openai.responses.create({
      ...
      input:[{
        role:"user",
        content:[
          {type:"input_text",text:"Identify the machinery model/serial plate in this image for REELMOW. Read only visible text. Do not guess missing characters. Return likely manufacturer, product family, model, variant, serial number and confidence. If uncertain, use null and explain the uncertainty."},
      ...
      text:{format:{type:"json_schema",name:"reelmow_machine_identification",strict:true,schema:{
        ...
        properties:{
          ...
          product_family:{type:"anyOf",items:[{type:"string"},{type:"null"}]},
          model:{type:"anyOf",items:[{type:"string"},{type:"null"}]},
          variant:{type:"anyOf",items:[{type:"string"},{type:"null"}]},
          serial_number:{type:"anyOf",items:[{type:"string"},{type:"null"}]},
