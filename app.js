...
function renderDetail(){
  ...
  mount(`<div class="breadcrumb"><button class="back" data-action="back">← Garage</button><span>/</span><span>Machine profile</span></div>
  <section class="detail-head"><div class="machine-title"><div class="machine-title-icon">⚙︎</div><div><div class="eyebrow">${esc(m.manufacturer?.name||"Manufacturer")}</div><h1 style="font-size:38px;margin-bottom:5px">${esc(m.nickname||m.model?.model_name||"Machine")}</h1><p class="muted">${esc(m.variant?.variant_name||"Variant")}</p></div></div><div class="actions"><span class="badge ${m.status==="service_due"?"service":"ready"}">${esc((m.status||"ready").replaceAll("_"," "))}</span></div></section>
  <div class="detail-grid"><div>
    <div class="card"><div class="section-head" style="margin:0 0 12px"><div><div class="eyebrow">Machine health</div><h2>At a glance</h2></div><button class="btn secondary small" data-action="edit">Edit</button></div>
      <div class="metric-row"><div class="metric"><div class="num">${m.current_engine_hours??"—"}</div><div class="label">Engine hours</div></div><div class="metric"><div class="num">${m.current_reel_hours??"—"}</div><div class="label">Reel hours</div></div></div>
      <div class="actions" style="margin-top:14px"><button class="btn small" data-action="hours">Update hours</button><button class="btn secondary small" data-action="service">Record service</button><button class="btn ghost small" data-action="fault">Report problem</button></div>
      <div class="list">
        <div class="list-row"><div><div class="list-title">Serial number</div><div class="list-meta">${esc(m.serial_number||"Not recorded")}</div></div></div>
        <div class="list-row"><div><div class="list-title">Asset number</div><div class="list-meta">${esc(m.asset_number||"Not recorded")}</div></div></div>
        <div class="list-row"><div><div class="list-title">Purchase</div><div class="list-meta">${esc(m.purchase_date||"Not recorded")}${m.purchase_price!=null?" · £"+Number(m.purchase_price).toLocaleString(undefined,{minimumFractionDigits:2,maximumFractionDigits:2}):""}</div></div></div>
        <div class="list-row"><div><div class="list-title">Ownership</div><div class="list-meta">${esc([m.ownership_type,m.ownership_name].filter(Boolean).join(" · ")||"Not recorded")}</div></div></div>
        <div class="list-row"><div><div class="list-title">Warranty</div><div class="list-meta">${esc([m.warranty_start_date,m.warranty_end_date,m.warranty_provider].filter(Boolean).join(" · ")||"Not recorded")}</div></div></div>
      </div>
      <div class="tiny" style="margin-top:12px">${esc(m.notes||"No machine notes recorded.")}</div>
    </div>
    <div class="card" style="margin-top:15px"><div class="eyebrow">Service status</div><h2>What needs doing?</h2><div id="service-due"><div class="loading"><div class="spinner"></div>Checking service schedule…</div></div></div>
    <div class="card" style="margin-top:15px"><div class="eyebrow">Catalogue specifications</div><h2>Known machine data</h2><div id="specs"><div class="loading"><div class="spinner"></div>Loading verified specifications…</div></div></div>
  </div><div>
    <div class="card"><div class="eyebrow">Service history</div><h2>Recent work</h2><div id="service-history"><div class="loading"><div class="spinner"></div>Loading service history…</div></div></div>
